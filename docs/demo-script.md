...
2. Request a fresh solvency attestation:
   ...
   - Status: **Solvent** (but close to threshold)
3. Show the proof being published on-chain:
   - `SolvencyRoot.publishRoot()` called with new Merkle root
   - Transaction hash and block number displayed
4. Verify the proof on-chain:
   - Anyone can call `SolvencyRoot.verifyProof()` with the Merkle proof
   - The auditor verifies without seeing any position amounts
5. Show the "wow moment": the auditor can verify the treasury is solvent without ever seeing a single position.

**Narration**:
> "An auditor can verify this treasury is solvent without ever seeing a single position. That is the confidentiality-to-verifiability transformation — and it is only possible on Flare. The TEE computes the proof, the Merkle root is published on-chain, and anyone can verify cryptographically."
...
| **How do you verify solvency without seeing positions?** | TEE computes the solvency margin and publishes a Merkle root. The auditor verifies the proof cryptographically using the Merkle root and individual proof paths. No position data is revealed. |
...
- [ ] All 5 vault contracts deployed (VaultCore, VerifierRole, PolicyRegistry, SolvencyRoot, InstructionSender)
- [ ] FDCAttestor and PMWInstructionRelay deployed
- [ ] System contracts accessible (FtsoV2, FdcHub, FdcVerification, FlareTeeManager)
- [ ] FCC extension compiles and runs (`go build ./cmd/server/`)
- [ ] All Go tests pass (`go test ./...`)
- [ ] All Foundry tests pass (`forge test --summary`)
- [ ] E2E flow test passes (`go test -v -run TestE2E_DepositRiskRebalanceAttestation ./internal/e2e/`)
- [ ] Failure-mode tests pass (TEE down, PMW failure, FDC delay)
- [ ] Dashboard loads and connects to Flare RPC
...
**If running over time**: Skip the Policy Engine detail in the rebalance section and compress the TEE attestation proof in the confidential position section. The "wow moment" is the auditor verification — do not cut that.

**If running under time**: Expand the TEE attestation walkthrough and show the Merkle proof verification in more detail. Add a brief mention of the error handling (safe-state logic, circuit breaker).
